...
orc_Mod_MAST_DEP_SwitchOff::orc_Mod_MAST_DEP_SwitchOff (/*ModuleTask* mt,
							  int indexclk*/)
  : ModuleAlgo (/*"orc_Mod_MAST_DEP_SwitchOff", mt, indexclk*/)
{
  //  PRINT1("** orc_Mod_MAST_DEP_SwitchOff constructor")
}

orc_Mod_MAST_DEP_SwitchOff::~orc_Mod_MAST_DEP_SwitchOff ()
{
  //  PRINT1( "** orc_Mod_MAST_DEP_SwitchOff destructor" )
}

void orc_Mod_MAST_DEP_SwitchOff::param (char *params)
{

  fprintf (stderr, "MAST_DEP_SwitchOff: param\n");

    targetMechanism = TARGET_MECHANISM_DEP;

    // the action status is ok
    action_exec_status = ACTION_RET_OK;
}

void orc_Mod_MAST_DEP_SwitchOff::reparam (char *params)
{
  //  fprintf( stderr, "MAST_DEP_SwitchOff: reparam\n" );
  // #include   "MAST_DEP_SwitchOff/user_code/reparam_code.c"
}

void orc_Mod_MAST_DEP_SwitchOff::init ()
{
  std::cerr << "MAST_DEP_SwitchOff: init" << std::endl;

	// reset the counter
	index = 0;

	// reset the events
	MAST_DEP_SwitchOff_prec = NO_EVENT;
	MAST_DEP_SwitchOff_post = NO_EVENT;
	...
	*/
	...
	// get the action Id
	rtId = 38; // Mt_ptr->GetRobotTaskPtr()->GetId();
	...
	MAST_DEP_SwitchOff_prec = SET_EVENT;
	//	moduleSendEvent("MAST_DEP_SwitchOff_prec;");

}

void orc_Mod_MAST_DEP_SwitchOff::compute ()
{
  //  std::cerr << "-> MAST_DEP_SwitchOff: start compute" << std::endl;
	/*
	 * Get the MastState variable
	 */
	if ( theRobotProcedure->GetParameters()->get("MastState", DOUBLE,
	...
	if (index >= (duration/0.2)) {
	  ...
	  MAST_DEP_SwitchOff_post = SET_EVENT;
	  // moduleSendEvent("MAST_DEP_SwitchOff_post;");
	  MastState[MAST_ACTION_ID_INDEX]  = rtId;
...
void orc_Mod_MAST_DEP_SwitchOff::end ()
{
  fprintf( stderr, "MAST_DEP_SwitchOff: End\n" );
...
// End class orc_Mod_MAST_DEP_SwitchOff
