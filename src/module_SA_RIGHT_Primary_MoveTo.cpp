...
orc_Mod_SA_RIGHT_Primary_MoveTo::~orc_Mod_SA_RIGHT_Primary_MoveTo ()
{
  //  PRINT1( "** orc_Mod_SA_RIGHT_Primary_MoveTo destructor" )
}
...
void orc_Mod_SA_RIGHT_Primary_MoveTo::reparam (char *params)
{
  //  fprintf( stderr, "SA_RIGHT_Primary_MoveTo:: reparam\n" );
 // #include   "SA_RIGHT_Primary_MoveTo/user_code/reparam_code.c"
}
...
void orc_Mod_SA_RIGHT_Primary_MoveTo::init ()
{
   std::cerr << "-> SA_RIGHT_Primary_MoveTo: start init" << std::endl;
	...
	if ( SolarArrayState[SA_RIGHT_PRIMARY_STATUS_INDEX] != SA_OPER_MODE_STNDBY ) {
	  ...
	  fprintf(stderr, "In SA_RIGHT_Primary_MoveTo::init() SA STATUS is not SA_OPER_MODE_STNDBY as expected\n");
	  //return;
	 }
	 */
	...
	SolarArrayState[SA_RIGHT_PRIMARY_STATUS_INDEX] = SA_OPER_MODE_MOVING; 
	...
	*/
	...
	//
	SA_RIGHT_Primary_MoveTo_prec = SET_EVENT;
	//	moduleSendEvent("SA_RIGHT_Primary_MoveTo_prec;");

	//	adeManagement.resetFlags();

}

void orc_Mod_SA_RIGHT_Primary_MoveTo::compute ()
{
  //  std::cerr << "-> SA_RIGHT_Primary_MoveTo: start compute" << std::endl;
  
  //  if (adeManagement.ActivateADE() == 1) 
  //    return;

  //  if (adeManagement.DeactivateADE("SA_RIGHT_Primary_MoveTo_post;") == 1) 
  //    return;
	...
	if (coordinatedMotion == 1) {
	  ...
	  if (currentTime <= trajDuration) {
	    ...
	    SA_RIGHT_Primary_MoveTo_post = SET_EVENT;
	    //moduleSendEvent("SA_RIGHT_Primary_MoveTo_post;");
...
// End class orc_Mod_SA_RIGHT_Primary_MoveTo 
