...
orc_Mod_DHS_HighPwr2Reduced::orc_Mod_DHS_HighPwr2Reduced (/*ModuleTask* mt,
							    int indexclk*/)
  : ModuleAlgo (/*"orc_Mod_DHS_HighPwr2Reduced", mt, indexclk*/)
{
  //  PRINT1("** orc_Mod_DHS_HighPwr2Reduced constructor")
}

orc_Mod_DHS_HighPwr2Reduced::~orc_Mod_DHS_HighPwr2Reduced ()
{
  //  PRINT1( "** orc_Mod_DHS_HighPwr2Reduced destructor" )
}

void orc_Mod_DHS_HighPwr2Reduced::param (char *params)
{

  //  fprintf (stderr, "DHS_HighPwr2Reduced:: param\n");

	strcpy(target_mode, "reduced");

}

void orc_Mod_DHS_HighPwr2Reduced::reparam (char *params)
{
  //  fprintf( stderr, "DHS_HighPwr2Reduced:: reparam\n" );
  //  #include   "DHS_HighPwr2Reduced/user_code/reparam_code.c"
}

void orc_Mod_DHS_HighPwr2Reduced::init ()
{
  //  std::cerr << "-> DHS_HighPwr2Reduced: start init" << std::endl;

	// example code
	index = 0;

	DHS_HighPwr2Reduced_prec = NO_EVENT;
	DHS_HighPwr2Reduced_post = NO_EVENT;
	...
	*/
	...
	//
	DHS_HighPwr2Reduced_prec = SET_EVENT;
	//	moduleSendEvent("DHS_HighPwr2Reduced_prec;");

}

void orc_Mod_DHS_HighPwr2Reduced::compute ()
{
  //  std::cerr << "-> DHS_HighPwr2Reduced: start compute" << std::endl;

	// Get Action ID
  rtId = 15; // Mt_ptr->GetRobotTaskPtr()->GetId();
	...
	if (index >= (duration/0.2))
	{
	  ...
	  DHS_HighPwr2Reduced_post = SET_EVENT;
	  //	  moduleSendEvent("DHS_HighPwr2Reduced_post;");
	}
...
void orc_Mod_DHS_HighPwr2Reduced::end ()
{
  //  fprintf( stderr, "DHS_HighPwr2Reduced:: End\n" );
	DHS_HighPwr2Reduced_prec = NO_EVENT;
	DHS_HighPwr2Reduced_post = NO_EVENT;
...
// End class orc_Mod_DHS_HighPwr2Reduced  
