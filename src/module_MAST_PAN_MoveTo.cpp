...
namespace ControllerModelNamespace {
  ...
  orc_Mod_MAST_PAN_MoveTo::orc_Mod_MAST_PAN_MoveTo(/*ModuleTask* mt, 
  ...
  //: ModuleAlgo("orc_Mod_MAST_PAN_MoveTo", mt, indexclk) 
{
  ...
  orc_Mod_MAST_PAN_MoveTo::~orc_Mod_MAST_PAN_MoveTo() {
    // PRINT1( "** orc_Mod_MAST_PAN_MoveTo destructor" )
  }
  ...
  void orc_Mod_MAST_PAN_MoveTo::param(char *params) {
    ...
    if (1 != sscanf(params, "%lf", &finalPose[1])) {
  ...
  void orc_Mod_MAST_PAN_MoveTo::reparam(char *params) {
  ...
  void orc_Mod_MAST_PAN_MoveTo::init() {
    ...
    if (action_exec_status != ACTION_RET_OK) {
      fprintf(stderr, "In MAST_PAN_MoveTo::init() action status = error\n");
      return;
    }
    ...
    */
    ...
    MastState[MAST_PAN_STATUS_INDEX] = MAST_OPER_MODE_PTU_MOVING;
    ...
    //
    MAST_PAN_MoveTo_prec = SET_EVENT;
    //    moduleSendEvent("MAST_PAN_MoveTo_prec;");
    
    //    adeManagement.resetFlags();
    
  }
  
  void orc_Mod_MAST_PAN_MoveTo::compute() {
    
    // if the action status is not ok the code is not executed
    if (action_exec_status != ACTION_RET_OK) {
      fprintf(stderr, "In MAST_PAN_MoveTo::compute() action status = error\n");
      return;
    }
    /*
    ...
    if (adeManagement.DeactivateADE("MAST_PAN_MoveTo_post;") == 1) 
      return;
    */
    //
	...
	currentPose[i] = finalPose[i];
      ...
      MAST_PAN_MoveTo_post = SET_EVENT;
      // moduleSendEvent("MAST_PAN_MoveTo_post;");
