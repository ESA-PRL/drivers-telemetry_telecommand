...
orc_Mod_SA_RIGHT_Secondary_Initialise::~orc_Mod_SA_RIGHT_Secondary_Initialise ()
{
  //  PRINT1( "** orc_Mod_SA_RIGHT_Secondary_Initialise destructor" )
}
  ...
  std::cerr << "-> SA_RIGHT_Secondary_Initialise: start init" << std::endl;
	...
	*/
	...
	if ( theRobotProcedure->GetParameters()->set( "SolarArrayState", DOUBLE,
						      MAX_STATE_SIZE, 0,
						      ( char * ) SolarArrayState ) == ERROR ) {
	  fprintf(stderr, "In SA_RIGHT_Secondary_Initialise::init() SolarArrayState not found\n");
	}
	...
	SA_RIGHT_Secondary_Initialise_prec = SET_EVENT;
	//	moduleSendEvent("SA_RIGHT_Secondary_Initialise_prec;");



}

void orc_Mod_SA_RIGHT_Secondary_Initialise::compute ()
{
  //  std::cerr << "-> SA_RIGHT_Secondary_Initialise: start compute" << std::endl;
	//
	// get the global state to initialise the local variable
	//
	if ( theRobotProcedure->GetParameters()->get( "SolarArrayState", DOUBLE,
	    ...
	    PowerState[ORC_HEATER_SADE_R_LINE_INDEX] < minOpVoltage) */)
	{
	  ...
	  fprintf(stderr, "In SA_RIGHT_Secondary_Initialise::compute() no power available for operation!\n");

	  // send the post-condition
	  SA_RIGHT_Secondary_Initialise_post = SET_EVENT;
	  //	  moduleSendEvent("SA_RIGHT_Secondary_Initialise_post;");
	  // save the state
	  if ( theRobotProcedure->GetParameters()->set( "SolarArrayState", DOUBLE,
							MAX_STATE_SIZE, 0,
							( char * ) SolarArrayState ) == ERROR ) {
	    fprintf(stderr, "In SA_RIGHT_Secondary_Initialise::compute() SolarArrayState not found\n");
	  }
	  // return
	  return;
	 }
...
// End class orc_Mod_SA_RIGHT_Secondary_Initialise
