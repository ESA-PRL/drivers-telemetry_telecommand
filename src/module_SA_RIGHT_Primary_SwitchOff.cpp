...
orc_Mod_SA_RIGHT_Primary_SwitchOff::~orc_Mod_SA_RIGHT_Primary_SwitchOff ()
{
  //  PRINT1( "** orc_Mod_SA_RIGHT_Primary_SwitchOff destructor" )
}
...
{
  //  fprintf( stderr, "SA_RIGHT_Primary_SwitchOff:: reparam\n" );
 // #include   "SA_RIGHT_Primary_SwitchOff/user_code/reparam_code.c"
}
...
void orc_Mod_SA_RIGHT_Primary_SwitchOff::init ()
{
	...
	*/
	...
	SA_RIGHT_Primary_SwitchOff_prec = SET_EVENT;

	//	moduleSendEvent("SA_RIGHT_Primary_SwitchOff_prec;");

}

void orc_Mod_SA_RIGHT_Primary_SwitchOff::compute ()
{
  //  std::cerr << "-> SA_RIGHT_Primary_SwitchOff: start compute" << std::endl;
	if ( theRobotProcedure->GetParameters()->get( "SolarArrayState", DOUBLE,
	                                              MAX_STATE_SIZE, 0,
	                                              ( char * ) SolarArrayState ) == ERROR ) {
	  //		theRobotProcedure->ExitOnMissingStateSymbol(
	...
	if (index >= 10) {
	  ...
	  SolarArrayState[SA_RIGHT_PRIMARY_STATUS_INDEX] = SA_OPER_MODE_OFF;

	  SA_RIGHT_Primary_SwitchOff_post = SET_EVENT;
	  //	  moduleSendEvent("SA_RIGHT_Primary_SwitchOff_post;");
	}
